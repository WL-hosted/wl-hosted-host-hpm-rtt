...
> **本工程（HPM Bootloader）引用说明**：此目录为 [armink/SFUD](https://github.com/armink/SFUD)
> 上游仓库的完整拷贝，固定于 commit `6b4bef82e6c603b783a17968f1b75da89cc5e2f8`，除 `.git` 外
> 内容未做改动。板级配置 `sfud_cfg.h` 在 `bootloader/config/sfud_cfg.h`（原上游
> `sfud/inc/sfud_cfg.h` 模板的定制，见文件内注释）；板级 SPI 移植在
> `bootloader/port/sfud/sfud_port.c`。
> 用途：在 Bootloader 中通过 SPI1 识别外挂 W25Q128 NOR Flash。
