...
#define BOARD_UART_IDLE_TRGM_OUTPUT_GPTMR_SYNCI                                \
    HPM_TRGM1_OUTPUT_SRC_GPTMR2_SYNCI
...
#define BOARD_UART_LIN_TX_PIN                                                  \
    (26U) /* PC26 should align with used pin in pinmux configuration */
...
/* LED: RGB 三灯,低电平点亮(共阳接法),见 documents/Hardware_PRD.md 3.8 */
#define BOARD_LED_GPIO_CTRL HPM_GPIO0
#define BOARD_LED_GPIO_INDEX GPIO_DI_GPIOC
#define BOARD_LED_R_PIN 23
#define BOARD_LED_G_PIN 22
#define BOARD_LED_B_PIN 21
...
void board_reconfig_console_baudrate(void);
...
uint32_t board_sd_configure_clock(
    SDXC_Type *ptr, uint32_t freq, bool need_inverse
);
...
void board_init_gptmr_channel_pin(
    GPTMR_Type *ptr, uint32_t channel, bool as_comp
);
