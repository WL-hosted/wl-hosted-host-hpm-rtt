...
void init_uart_pin_as_gpio(UART_Type *ptr) {
    if (ptr == HPM_UART2) {
        /* pull-up */
        HPM_IOC->PAD[IOC_PAD_PC26].PAD_CTL =
            IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_PS_SET(1);
        HPM_IOC->PAD[IOC_PAD_PC27].PAD_CTL =
            IOC_PAD_PAD_CTL_PE_SET(1) | IOC_PAD_PAD_CTL_PS_SET(1);
...
void init_i2c_pins(I2C_Type *ptr) {
    if (ptr == HPM_I2C0) {
        HPM_IOC->PAD[IOC_PAD_PC13].FUNC_CTL =
            IOC_PC13_FUNC_CTL_I2C0_SCL | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
        HPM_IOC->PAD[IOC_PAD_PC14].FUNC_CTL =
            IOC_PC14_FUNC_CTL_I2C0_SDA | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
        HPM_IOC->PAD[IOC_PAD_PC13].PAD_CTL = IOC_PAD_PAD_CTL_OD_SET(1) |
                                             IOC_PAD_PAD_CTL_PE_SET(1) |
                                             IOC_PAD_PAD_CTL_PS_SET(1);
        HPM_IOC->PAD[IOC_PAD_PC14].PAD_CTL = IOC_PAD_PAD_CTL_OD_SET(1) |
                                             IOC_PAD_PAD_CTL_PE_SET(1) |
                                             IOC_PAD_PAD_CTL_PS_SET(1);
    } else if (ptr == HPM_I2C3) {
        HPM_IOC->PAD[IOC_PAD_PC11].FUNC_CTL =
            IOC_PC11_FUNC_CTL_I2C3_SCL | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
        HPM_IOC->PAD[IOC_PAD_PC12].FUNC_CTL =
            IOC_PC12_FUNC_CTL_I2C3_SDA | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
        HPM_IOC->PAD[IOC_PAD_PC11].PAD_CTL = IOC_PAD_PAD_CTL_OD_SET(1) |
                                             IOC_PAD_PAD_CTL_PE_SET(1) |
                                             IOC_PAD_PAD_CTL_PS_SET(1);
        HPM_IOC->PAD[IOC_PAD_PC12].PAD_CTL = IOC_PAD_PAD_CTL_OD_SET(1) |
                                             IOC_PAD_PAD_CTL_PE_SET(1) |
                                             IOC_PAD_PAD_CTL_PS_SET(1);
    } else {
        while (1) {
...
void init_psram_pins(void) {
    /* PSRAM 使用 XPI1 CA/CS0，第一引脚组，无 DQS；PB11 保持 UART6 RX。 */
    HPM_IOC->PAD[IOC_PAD_PB12].FUNC_CTL = IOC_PB12_FUNC_CTL_XPI1_CA_D_0;
    HPM_IOC->PAD[IOC_PAD_PB15].FUNC_CTL = IOC_PB15_FUNC_CTL_XPI1_CA_D_1;
    HPM_IOC->PAD[IOC_PAD_PB13].FUNC_CTL = IOC_PB13_FUNC_CTL_XPI1_CA_D_2;
    HPM_IOC->PAD[IOC_PAD_PB16].FUNC_CTL = IOC_PB16_FUNC_CTL_XPI1_CA_D_3;
    HPM_IOC->PAD[IOC_PAD_PB14].FUNC_CTL = IOC_PB14_FUNC_CTL_XPI1_CA_SCLK;
    HPM_IOC->PAD[IOC_PAD_PB17].FUNC_CTL = IOC_PB17_FUNC_CTL_XPI1_CA_CS0;

    const uint32_t pad_ctl = IOC_PAD_PAD_CTL_SR_MASK | IOC_PAD_PAD_CTL_SPD_SET(3);
...
void init_spi_pins(SPI_Type *ptr) {
    ...
    case (uint32_t)HPM_SPI3:
        ...
        HPM_IOC->PAD[IOC_PAD_PC20].FUNC_CTL =
            IOC_PC20_FUNC_CTL_SPI3_SCLK | IOC_PAD_FUNC_CTL_LOOP_BACK_SET(1);

        /* set max frequency slew rate(200M) */
        HPM_IOC->PAD[IOC_PAD_PC18].PAD_CTL =
        ...
        break;
    default:
        LOG_W("unsupported SPI instance: 0x%lx", (uint32_t)ptr);
        break;
    }
}
...
void init_spi_pins_with_gpio_as_cs(SPI_Type *ptr) {
    if (ptr == HPM_SPI3) {
        ...
        /* SPI1 A 组引脚：CSN=PA16(GPIO 软件 CS，由 RT-Thread SPI 框架配置)、
         * SCLK=PA18、MOSI=PA19、MISO=PA17，复用 ALT5 */
...
void init_gptmr_pins(GPTMR_Type *ptr) {
    if (ptr == HPM_GPTMR2) {
        ...
        trgm_output_config(
            HPM_TRGM1, HPM_TRGM1_OUTPUT_SRC_TRGM1_P4, &trgm1_io_config0
        );
...
void init_enet_pins(ENET_Type *ptr) {
    if (ptr == HPM_ENET0) {
        ...
        HPM_IOC->PAD[IOC_PAD_PA22].FUNC_CTL =
            IOC_PAD_FUNC_CTL_LOOP_BACK_MASK | IOC_PA22_FUNC_CTL_ETH0_REFCLK;
    }
}
...
void init_sdxc_cmd_pin(SDXC_Type *ptr, bool open_drain, bool is_1v8) {
    (void)is_1v8;
    if (ptr == HPM_SDXC0) {
        uint32_t cmd_func_ctl =
            IOC_PA10_FUNC_CTL_SDC0_CMD | IOC_PAD_FUNC_CTL_LOOP_BACK_SET(1);
        uint32_t cmd_pad_ctl = IOC_PAD_PAD_CTL_DS_SET(7) |
                               IOC_PAD_PAD_CTL_PE_SET(1) |
                               IOC_PAD_PAD_CTL_PS_SET(1);
        if (open_drain) {
...
void init_sdxc_cd_pin(SDXC_Type *ptr, bool as_gpio) {
    if (ptr == HPM_SDXC0) {
        uint32_t cd_func_alt =
            as_gpio ? IOC_PA14_FUNC_CTL_GPIO_A_14 : IOC_PA14_FUNC_CTL_SDC0_CDN;
        uint32_t cd_pad_ctl = IOC_PAD_PAD_CTL_DS_SET(7) |
                              IOC_PAD_PAD_CTL_PE_SET(1) |
                              IOC_PAD_PAD_CTL_PS_SET(1);
        /* SDXC0.CD */
...
void init_sdxc_clk_data_pins(SDXC_Type *ptr, uint32_t width, bool is_1v8) {
    ...
    if (ptr == HPM_SDXC0) {
        ...
        uint32_t pad_ctl = IOC_PAD_PAD_CTL_DS_SET(7) |
                           IOC_PAD_PAD_CTL_PE_SET(1) |
                           IOC_PAD_PAD_CTL_PS_SET(1);
...
void init_led_pins(void) {
    /* RGB LED: R=PC23, G=PC22, B=PC21(低电平点亮),见 Hardware_PRD.md 3.8 */
...
void init_tamper_pins(void) {
    HPM_BIOC->PAD[IOC_PAD_PZ06].FUNC_CTL =
        BIOC_PZ06_FUNC_CTL_TAMP_06 | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
...
void init_gptmr_channel_pin(GPTMR_Type *ptr, uint32_t channel, bool as_comp) {
    if (ptr == HPM_GPTMR2) {
        if (as_comp) {
            switch (channel) {
            case 0:
                HPM_IOC->PAD[IOC_PAD_PC08].FUNC_CTL =
                    IOC_PC08_FUNC_CTL_GPTMR2_COMP_0;
                break;
            case 1:
                HPM_IOC->PAD[IOC_PAD_PC09].FUNC_CTL =
                    IOC_PC09_FUNC_CTL_GPTMR2_COMP_1;
                break;
            case 2:
                HPM_IOC->PAD[IOC_PAD_PA24].FUNC_CTL =
                    IOC_PA24_FUNC_CTL_TRGM1_P_04;
                trgm_output_t trgm1_io_config0 = {0};
                trgm1_io_config0.invert = 0;
                trgm1_io_config0.type = trgm_output_same_as_input;
                trgm1_io_config0.input = HPM_TRGM1_INPUT_SRC_GPTMR2_OUT2;
            ...
            if (channel == 0) {
                HPM_IOC->PAD[IOC_PAD_PC06].FUNC_CTL =
                    IOC_PC06_FUNC_CTL_GPTMR2_CAPT_0;
            }
        }
    }
}
