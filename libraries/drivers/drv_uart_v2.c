...
#define UART_RX_DMA_ABORT_RETRY (1000U)
...
static rt_err_t hpm_uart_receive_dma_next(struct rt_serial_device *serial);

#if !defined(HPM_IP_FEATURE_UART_RX_IDLE_DETECT) || (HPM_IP_FEATURE_UART_RX_IDLE_DETECT != 1)
static rt_err_t hpm_uart_get_rx_dma_buffer(struct rt_serial_device *serial,
                                           uint8_t **buffer,
                                           uint32_t *size)
{
    if ((serial == RT_NULL) || (serial->serial_rx == RT_NULL) ||
        (buffer == RT_NULL) || (size == RT_NULL)) {
        return -RT_ERROR;
    }

    *buffer = RT_NULL;
    *size = serial->config.dma_ping_bufsz;
    if ((*size == 0U) ||
        (rt_hw_serial_control_isr(serial, RT_HW_SERIAL_CTRL_GET_DMA_PING_BUF, buffer) != RT_EOK) ||
        (*buffer == RT_NULL)) {
        return -RT_ERROR;
    }
...
struct hpm_uart {
    ...
    uint32_t rx_dma_active_size;
    bool rx_dma_flush_in_progress;
#endif
...
static int hpm_uart_getc(struct rt_serial_device *serial);
    ...
    (!defined(HPM_IP_FEATURE_UART_RX_IDLE_DETECT) || (HPM_IP_FEATURE_UART_RX_IDLE_DETECT != 1))
static void hpm_uart_invalidate_rx_dma_buffer(uint8_t *buffer, uint32_t size)
{
    if ((buffer == RT_NULL) || (size == 0U) || !l1c_dc_is_enabled()) {
        return;
    }

    uint32_t aligned_start = HPM_L1C_CACHELINE_ALIGN_DOWN((uint32_t)buffer);
    uint32_t aligned_end = HPM_L1C_CACHELINE_ALIGN_UP((uint32_t)buffer + size);
    l1c_dc_invalidate(aligned_start, aligned_end - aligned_start);
}

static rt_err_t hpm_uart_wait_rx_dma_stopped(struct hpm_uart *uart)
{
    uint32_t channel_mask = 1UL << uart->rx_chn_ctx.resource.channel;
    uint32_t retry = UART_RX_DMA_ABORT_RETRY;

    while ((dma_check_enabled_channel(uart->rx_chn_ctx.resource.base, channel_mask) != 0U) &&
           (retry > 0U)) {
        --retry;
    }

    return (dma_check_enabled_channel(uart->rx_chn_ctx.resource.base, channel_mask) == 0U) ?
        RT_EOK : -RT_ETIMEOUT;
}

static rt_err_t hpm_uart_rx_dma_timeout(struct rt_serial_device *serial)
{
    struct hpm_uart *uart = (struct hpm_uart *)serial->parent.user_data;
    dma_resource_t *resource = &uart->rx_chn_ctx.resource;
    ...
    if ((serial->serial_rx == RT_NULL) || uart->rx_dma_flush_in_progress ||
        (uart->rx_dma_active_size == 0U)) {
        return -RT_ERROR;
    }

    uart->rx_dma_flush_in_progress = true;
    dma_mgr_disable_chn_irq(resource, DMA_MGR_INTERRUPT_MASK_TC);
    dma_abort_channel(resource->base, 1UL << resource->channel);
    if (hpm_uart_wait_rx_dma_stopped(uart) != RT_EOK) {
        uart->rx_dma_active_size = 0U;
        uart->rx_dma_flush_in_progress = false;
        return -RT_ETIMEOUT;
    }

    remaining_size = dma_get_remaining_transfer_size(resource->base, resource->channel);
    dma_clear_transfer_status(resource->base, resource->channel);
    if (remaining_size > uart->rx_dma_active_size) {
        uart->rx_dma_active_size = 0U;
        uart->rx_dma_flush_in_progress = false;
        return -RT_ERROR;
    }

    received_size = uart->rx_dma_active_size - remaining_size;
    uart->rx_dma_active_size = 0U;
    if (hpm_uart_get_rx_dma_buffer(serial, &rx_dma_buffer, &rx_dma_buffer_size) != RT_EOK) {
        uart->rx_dma_flush_in_progress = false;
        return -RT_ERROR;
    }

    if ((received_size > 0U) &&
        ((uart->rx_dma_offset + received_size) <= rx_dma_buffer_size)) {
        hpm_uart_invalidate_rx_dma_buffer(rx_dma_buffer + uart->rx_dma_offset, received_size);
        uart->rx_dma_offset += received_size;
        if (uart->rx_dma_offset == rx_dma_buffer_size) {
            uart->rx_dma_offset = 0U;
        }
        rt_hw_serial_isr(serial, RT_SERIAL_EVENT_RX_DMADONE | (received_size << 8));
        if (serial->serial_rx == RT_NULL) {
            uart->rx_dma_flush_in_progress = false;
            return -RT_ERROR;
        }
    } else if (received_size > 0U) {
        uart->rx_dma_flush_in_progress = false;
        return -RT_ERROR;
    }

    while (uart_check_status(uart->uart_base, uart_stat_data_ready) &&
           (fifo_count < UART_SOC_FIFO_SIZE)) {
        uint8_t ch = uart_read_byte(uart->uart_base);
        (void)rt_hw_serial_control_isr(serial, RT_HW_SERIAL_CTRL_PUTC, &ch);
        ++fifo_count;
    }
    if (fifo_count > 0U) {
    ...
    if (hpm_uart_receive_dma_next(serial) != RT_EOK) {
        uart->rx_dma_flush_in_progress = false;
        return -RT_ERROR;
    }
    uart->rx_dma_flush_in_progress = false;
...
static void uart_dma_tc_callback(DMA_Type *base, uint32_t channel, void *user_data)
{
    ...
    if (dma_handle->tranfer_done != RT_NULL) {
...
static void uart_dma_abort_callback(DMA_Type *base, uint32_t channel, void *user_data)
{
    ...
    if (dma_handle->tranfer_abort != RT_NULL) {
...
static void uart_dma_error_callback(DMA_Type *base, uint32_t channel, void *user_data)
{
    ...
    if (dma_handle->tranfer_error != RT_NULL) {
...
static void uart_rx_done(struct rt_serial_device *serial)
{
    ...
    if (uart->rx_dma_flush_in_progress || (uart->rx_dma_active_size == 0U)) {
        return;
    }
    if (hpm_uart_get_rx_dma_buffer(serial, &rx_dma_buffer, &rx_dma_buffer_size) != RT_EOK) {
        return;
    }
    received_size = uart->rx_dma_active_size;
    if ((uart->rx_dma_offset + received_size) > rx_dma_buffer_size) {
        uart->rx_dma_active_size = 0U;
        return;
    }
    hpm_uart_invalidate_rx_dma_buffer(rx_dma_buffer + uart->rx_dma_offset, received_size);
    ...
    (void)hpm_uart_receive_dma_next(serial);
}
#endif /* RT_SERIAL_USING_DMA */
...
static void hpm_uart_isr(struct rt_serial_device *serial)
{
...
#if defined(RT_SERIAL_USING_DMA) && \
    (!defined(HPM_IP_FEATURE_UART_RX_IDLE_DETECT) || (HPM_IP_FEATURE_UART_RX_IDLE_DETECT != 1))
        /* ERBI controls both RX data and character timeout. In DMA mode the
         * data-available condition belongs to DMA; the CPU only drains the
         * FIFO after the timeout path has stopped DMA. */
        if ((uart->dma_flags & RT_DEVICE_FLAG_DMA_RX) == 0U)
#endif
        {
            while (uart_check_status(uart->uart_base, uart_stat_data_ready)) {
                count++;
    ...
    (!defined(HPM_IP_FEATURE_UART_RX_IDLE_DETECT) || (HPM_IP_FEATURE_UART_RX_IDLE_DETECT != 1))
        if (uart->dma_flags & RT_DEVICE_FLAG_DMA_RX) {
            (void)hpm_uart_rx_dma_timeout(serial);
        } else
#endif
        {
            while ((uart_check_status(uart->uart_base, uart_stat_data_ready)) ||
                   (uart_check_status(uart->uart_base, uart_stat_overrun_error))) {
                put_char = uart_read_byte(uart->uart_base);
                rt_ringbuffer_putchar(&(rx_fifo->rb), put_char);
            }
            rt_hw_serial_isr(serial, RT_SERIAL_EVENT_RX_IND);
    ...
    if ((irq_id == uart_intr_id_tx_slot_avail) && (stat & uart_stat_tx_slot_avail)) {
...
#else
            /* Leave up to three bytes in the 16-byte FIFO so the standard
             * character-timeout interrupt can flush a partial DMA block. */
            uart_config.rx_fifo_level = uart_rx_fifo_trg_gt_one_quarter;
#endif
        }
#if defined(HPM_IP_FEATURE_UART_RX_IDLE_DETECT) && (HPM_IP_FEATURE_UART_RX_IDLE_DETECT == 1)
...
static int hpm_uart_dma_config(struct rt_serial_device *serial, void *arg)
{
    ...
    if (ctrl_arg == RT_DEVICE_FLAG_DMA_RX) {
        ...
        uart->rx_dma_offset = 0U;
        uart->rx_dma_active_size = rx_dma_buffer_size;
        uart->rx_dma_flush_in_progress = false;
        chg_config.dst_addr = (uint32_t)rx_dma_buffer;
        chg_config.size_in_byte = rx_dma_buffer_size;
#endif
        ...
        dma_clear_transfer_status(uart->rx_chn_ctx.resource.base, uart->rx_chn_ctx.resource.channel);
...
static rt_err_t hpm_uart_receive_dma_next(struct rt_serial_device *serial)
{
    ...
    buf_addr = (uint32_t)(rx_dma_buffer + uart->rx_dma_offset);
    buf_size = rx_dma_buffer_size - uart->rx_dma_offset;
    uart->rx_dma_active_size = buf_size;
#endif
    ...
    switch (cmd) {
        case RT_DEVICE_CTRL_CLR_INT:
            if (ctrl_arg == RT_DEVICE_FLAG_INT_RX) {
                ...
                dma_abort_channel(uart->tx_chn_ctx.resource.base,
                                  1UL << uart->tx_chn_ctx.resource.channel);
                ...
                dma_abort_channel(uart->rx_chn_ctx.resource.base,
                                  1UL << uart->rx_chn_ctx.resource.channel);
#if !defined(HPM_IP_FEATURE_UART_RX_IDLE_DETECT) || (HPM_IP_FEATURE_UART_RX_IDLE_DETECT != 1)
                (void)hpm_uart_wait_rx_dma_stopped(uart);
                dma_clear_transfer_status(uart->rx_chn_ctx.resource.base,
                                          uart->rx_chn_ctx.resource.channel);
                uart->rx_dma_offset = 0U;
                uart->rx_dma_active_size = 0U;
                uart->rx_dma_flush_in_progress = false;
#endif
        ...
        case RT_DEVICE_CTRL_CONFIG:
            ...
            if ((ctrl_arg == RT_DEVICE_FLAG_DMA_RX) || (ctrl_arg == RT_DEVICE_FLAG_INT_RX)) {
                if ((rx_fifo) &&
                    (((rt_uint32_t)rx_fifo->rb.buffer_ptr % HPM_L1C_CACHELINE_SIZE) ||
                     (rx_fifo->rb.buffer_size % HPM_L1C_CACHELINE_SIZE) ||
                     ((rt_uint32_t)rx_fifo->dma_ping_rb.buffer_ptr % HPM_L1C_CACHELINE_SIZE) ||
                     (rx_fifo->dma_ping_rb.buffer_size % HPM_L1C_CACHELINE_SIZE))) {
                    rt_int32_t rx_timeout = rt_atomic_load(&rx_fifo->rx_timeout);
                    rt_size_t rx_cpt_index = rx_fifo->rx_cpt_index;
                    rt_free(rx_fifo);
                    rx_fifo = RT_NULL;
                    rx_fifo = (struct rt_serial_rx_fifo *) rt_malloc(sizeof(struct rt_serial_rx_fifo));
                    RT_ASSERT(rx_fifo != RT_NULL);
                    rt_memset(rx_fifo, 0, sizeof(struct rt_serial_rx_fifo));
                    tmp_buffer = rt_malloc(serial->config.rx_bufsz + serial->config.dma_ping_bufsz +
                                           2U * HPM_L1C_CACHELINE_SIZE);
                    ...
                    dma_ping_buffer = (uint8_t *)HPM_L1C_CACHELINE_ALIGN_UP(
                        (rt_ubase_t)tmp_buffer + serial->config.rx_bufsz);
                    rt_ringbuffer_init(&rx_fifo->rb, tmp_buffer, serial->config.rx_bufsz);
                    rt_ringbuffer_init(&rx_fifo->dma_ping_rb, dma_ping_buffer, serial->config.dma_ping_bufsz);
                    ...
                    if (serial->parent.open_flag & RT_SERIAL_RX_BLOCKING) {
                        ...
                        rx_fifo->rx_cpt_index = rx_cpt_index;
                        rt_atomic_store(&rx_fifo->rx_timeout, rx_timeout);
                    }
            ...
            if (ctrl_arg & (RT_DEVICE_FLAG_DMA_RX | RT_DEVICE_FLAG_DMA_TX)) {
                return hpm_uart_dma_config(serial, (void *)ctrl_arg);
            } else
#endif
            {
...
#ifdef RT_SERIAL_USING_DMA
    /* DMA RX buffers are invalidated by cache line. Keep each transfer in an
     * independently aligned cache line so adjacent serial state is untouched. */
    config.dma_ping_bufsz = HPM_L1C_CACHELINE_SIZE;
#endif
