 ...
 */
#if defined(FLASH_XIP) && FLASH_XIP && !(defined(FLASH_UF2) && FLASH_UF2)
__attribute__((section(".nor_cfg_option"), used))
const uint32_t option[4] = {0xfcf90001, 0x00000007, 0x0, 0x0};
#endif
...
void board_init_console(void) {
    ...
    level change of uart rx pin when configuring pin function will cause a wrong
    data to be received. And a uart rx dma request will be generated by default
    uart fifo dma trigger level. */
...
#if !defined(CONFIG_NDEBUG_CONSOLE) || !CONFIG_NDEBUG_CONSOLE
#if defined(RT_USING_CONSOLE) && (CONSOLE_TYPE_UART == BOARD_CONSOLE_TYPE)
    /* 串口框架注册设备时按 RT_SERIAL_CONFIG_DEFAULT(115200)重新初始化了
     * UART,这里把控制台串口重配回板级波特率,使 rt_show_version 的 banner
     * 与 Bootloader 日志口同波特率,且不影响其他串口的默认配置。
     * 必须在 rt_hw_uart_init() 之后、finsh 打开控制台之前调用:SDK 的
     * uart_init 会清零 IER,中断使能后再调用会丢失 RX 中断。 */
    rt_device_t console_dev = rt_device_find(RT_CONSOLE_DEVICE_NAME);
    if (console_dev != RT_NULL) {
        struct serial_configure cfg = RT_SERIAL_CONFIG_DEFAULT;
        cfg.baud_rate = BOARD_CONSOLE_UART_BAUDRATE;
...
void board_ungate_mchtmr_at_lp_mode(void) {
    ...
    sysctl_set_cpu_lp_mode(
        HPM_SYSCTL, BOARD_RUNNING_CORE, cpu_lp_mode_ungate_cpu_clock
    );
}
...
void board_timer_create(uint32_t ms, board_timer_cb cb) {
    ...
    gptmr_channel_config(
        BOARD_CALLBACK_TIMER, BOARD_CALLBACK_TIMER_CH, &config, false
    );
    gptmr_enable_irq(
        BOARD_CALLBACK_TIMER, GPTMR_CH_RLD_IRQ_MASK(BOARD_CALLBACK_TIMER_CH)
    );
...
void board_i2c_bus_clear(I2C_Type *ptr) {
    ...
    if (ptr == BOARD_APP_I2C_BASE) {
        ...
        gpio_set_pin_input(
            BOARD_I2C_GPIO_CTRL,
            BOARD_I2C_SCL_GPIO_INDEX,
            BOARD_I2C_SCL_GPIO_PIN
        );
        if (!gpio_read_pin(
                BOARD_I2C_GPIO_CTRL,
                BOARD_I2C_SCL_GPIO_INDEX,
...
uint32_t board_init_spi_clock(SPI_Type *ptr) {
    ...
    if (ptr == HPM_SPI1) {
        /* SPI1 默认时钟源 PLL0CLK0/5 = 80MHz，SFUD 侧按 RT_SFUD_SPI_MAX_HZ 取整倍分频 */
...
void board_init_spi_pins_with_gpio_as_cs(SPI_Type *ptr) {
    ...
    gpio_set_pin_output_with_initial(
        ...
        GPIO_GET_PORT_INDEX(BOARD_SPI_CS_PIN),
...
void board_led_toggle(uint8_t pin) {
    gpio_toggle_pin(
        BOARD_LED_GPIO_CTRL, BOARD_LED_GPIO_INDEX, pin
    );
}

void board_led_write(uint8_t pin, uint8_t state) {
    gpio_write_pin(
        BOARD_LED_GPIO_CTRL, BOARD_LED_GPIO_INDEX, pin, state
    );
}
...
uint32_t board_sd_configure_clock(
    ...
    do {
        ...
        /* Configure the clock to 50MHz for the SDR25/High speed/50MHz DDR/50MHz
           SDR */
        ...
        actual_freq =
            clock_get_frequency(sdxc_clk) / sdxc_get_clock_divider(ptr);
    } while (false);
...
hpm_stat_t board_init_enet_ptp_clock(ENET_Type *ptr) {
    ...
    if (ptr == HPM_ENET0) {
        ...
        /* make sure pll0_clk0 output clock at 400MHz to get a clock at 100MHz
         * for ent0 ptp clock */
...
hpm_stat_t board_init_enet_rmii_reference_clock(ENET_Type *ptr, bool internal) {
    ...
    if (ptr == HPM_ENET0) {
        ...
        if (internal) {
            ...
            if (pllctlv2_init_pll_with_freq(
                    HPM_PLLCTLV2, pllctlv2_pll2, 1000000000UL
                ) == status_success) {
                /* set pll2_clk1 output frequency at 250MHz from PLL2 divided by
                 * 4 (1 + 15 / 5) */
                pllctlv2_set_postdiv(
                    HPM_PLLCTLV2, pllctlv2_pll2, pllctlv2_clk1, pllctlv2_div_4p0
                );
