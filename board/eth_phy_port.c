...
eth_phy_monitor_handle_t phy_monitor_handle = {
    .phy_handle_cnt = ARRAY_SIZE(s_gphys), .phy_handle = s_gphys
};

static struct rt_phy_ops phy_ops;

static rt_phy_status phy_init(
    void *object, rt_uint32_t phy_addr, rt_uint32_t src_clock_hz
) {
    return PHY_STATUS_OK;
}

static rt_size_t phy_read(
    void *bus, rt_uint32_t addr, rt_uint32_t reg, void *data, rt_uint32_t size
) {
    *(uint16_t *)data =
        enet_read_phy(((struct rt_mdio_bus *)bus)->hw_obj, addr, reg);

    return size;
}

static rt_size_t phy_write(
    void *bus, rt_uint32_t addr, rt_uint32_t reg, void *data, rt_uint32_t size
) {
    enet_write_phy(
        ((struct rt_mdio_bus *)bus)->hw_obj, addr, reg, *(uint16_t *)data
    );
...
static rt_phy_status phy_get_link_status(rt_phy_t *phy, rt_bool_t *status) {
    ...
    reg_status = enet_read_phy(
        phy->bus->hw_obj, phy->addr, phy->reg_list[PHY_BASIC_STATUS_REG_IDX]
    );
...
static rt_phy_status phy_get_link_speed_duplex(
    rt_phy_t *phy, rt_uint32_t *speed, rt_uint32_t *duplex
) {
    uint16_t reg_status;

    reg_status = enet_read_phy(
        phy->bus->hw_obj, phy->addr, phy->reg_list[PHY_STATUS_REG_IDX]
    );

    *speed = PHY_STATUS_SPEED_100M(reg_status) ? PHY_SPEED_100M : PHY_SPEED_10M;
    *duplex =
        PHY_STATUS_FULL_DUPLEX(reg_status) ? PHY_FULL_DUPLEX : PHY_HALF_DUPLEX;
...
static void phy_poll_status(void *parameter) {
    ...
    eth_phy_monitor_handle_t *phy_monitor_handle =
        (eth_phy_monitor_handle_t *)parameter;
    ...
    for (uint32_t i = 0; i < phy_monitor_handle->phy_handle_cnt; i++) {
        ...
        if (status) {
            phy_dev->phy.ops->get_link_speed_duplex(
                &phy_dev->phy, &phy_info.phy_speed, &phy_info.phy_duplex
            );
        ...
        if (phy_dev->phy_link != status) {
            ...
            if (status == PHY_LINK_UP) {
                ...
                LOG_I(
                    "PHY Duplex: %s\n",
                    phy_dev->phy_info.phy_duplex & PHY_FULL_DUPLEX
                        ? "full duplex"
                        : "half duplex"
                );
                enet_set_line_speed(
                    phy_monitor_handle->phy_handle[i]->instance,
                    line_speed[phy_dev->phy_info.phy_speed]
                );
...
static void phy_detection(void *parameter) {
    ...
    phy_dev->phy.ops->init(
        phy_dev->phy.bus->hw_obj, phy_dev->phy.addr, PHY_MDIO_CSR_CLK_FREQ
    );
    ...
    while (phy_dev->phy.addr == 0xffff) {
        ...
        for (i = 0; i <= 0x1f; i++) {
            ...
            if (msg.value == PHY_ID1) {
                phy_dev->phy.addr = i;
                LOG_D(
                    "Found a PHY device[address:0x%02x].\n", phy_dev->phy.addr
                );
                return;
            }
        }
        ...
        if (detected_count > 3) {
            LOG_E(
                "No any PHY device is detected! Please check your hardware!\n"
            );
            return;
        }
    }
...
static void phy_monitor_thread_entry(void *args) {
    ...
    phy_status_timer = rt_timer_create(
        "PHY_Monitor",
        phy_poll_status,
        phy_monitor_handle,
        RT_TICK_PER_SECOND,
...
int phy_device_register(void) {
    ...
    /* Start PHY monitor */
    thread_phy_monitor = rt_thread_create(
        "PHY Monitor",
