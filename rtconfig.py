...
CROSS_TOOL='gcc'
GCC_PATH = "/Users/kai/DevTools/xpack-riscv-none-elf-gcc-15.2.0-1/bin"
...
if PLATFORM == 'gcc':
    ...
    GDB = PREFIX + 'gdb'
    TARGET_EXT = 'elf'
    # 统一产物命名：ELF/BIN 均由此派生（SConstruct 的 TARGET 与 POST_ACTION 引用）
    ...
    ARCH_ABI = ' -mcmodel=medlow -march=rv32imac_zicsr_zifencei -mabi=ilp32 '
    disable_wdg = False
    DEVICE = ARCH_ABI  + ' -DUSE_NONVECTOR_MODE=1' + ' -ffunction-sections -fdata-sections -fno-common '
    if disable_wdg:
        DEVICE += ' -DDISABLE_WDG=1 '
    CFLAGS = DEVICE
    AFLAGS = CFLAGS
    LFLAGS  = ARCH_ABI + '  --specs=nano.specs --specs=nosys.specs  -u _printf_float -u _scanf_float -nostartfiles -Wl,--gc-sections -Wl,-Map=' + TARGET_NAME + '.map '
    ...
    if BUILD == 'ram_debug':
        ...
        CFLAGS += ' -O0'
        LFLAGS += ' -O0'
        CFLAGS += ' -DFLASH_XIP=1 -DFLASH_UF2=1'
        LINKER_FILE = 'board/linker_scripts/gcc/flash_rtt_uf2.ld'
    ...
    # module setting
    CXXFLAGS = CFLAGS +  ' -std=gnu++20 -Woverloaded-virtual -fno-exceptions -fno-rtti '
