...
#define MCAUSE_ECALL_FROM_SUPERVISOR_MODE                                      \
    (9U) //!< Environment call from Supervisor mode
#define MCAUSE_ECALL_FROM_MACHINE_MODE                                         \
    (11U)                             //!< Environment call from machine mode
...
__attribute__((weak)) void syscall_handler(
    uint32_t n, uint32_t a0, uint32_t a1, uint32_t a2, uint32_t a3
) {
}
...
uint32_t exception_handler(uint32_t cause, uint32_t epc) {
    ...
    case MCAUSE_INSTR_ADDR_MISALIGNED:
        RT_EXCEPTION_TRACE(
            "exception: instruction address was mis-aligned, mtval=0x%08x\n",
            mtval
        );
        break;
    case MCAUSE_INSTR_ACCESS_FAULT:
        RT_EXCEPTION_TRACE(
            "exception: instruction access fault happened, mtval=0x%08x, "
            "epc=0x%08x\n",
            mtval,
            epc
        );
        ...
        case 2:
            RT_EXCEPTION_TRACE(
                "mdcause: PMP instruction access violation \r\n"
            );
            break;
        case 3:
    ...
    case MCAUSE_ILLEGAL_INSTR:
        RT_EXCEPTION_TRACE(
            "exception: illegal instruction was met, mtval=0x%08x\n", mtval
        );
        switch (mdcause & 0x07) {
        case 0:
            RT_EXCEPTION_TRACE(
                "mdcause: the actual faulting instruction is stored in the "
                "mtval CSR\r\n"
            );
            break;
        case 1:
    ...
    case MCAUSE_BREAKPOINT:
        RT_EXCEPTION_TRACE(
            "exception: breakpoint was hit, mtval=0x%08x\n", mtval
        );
        break;
    case MCAUSE_LOAD_ADDR_MISALIGNED:
        RT_EXCEPTION_TRACE(
            "exception: load address was mis-aligned, mtval=0x%08x\n", mtval
        );
        break;
    case MCAUSE_LOAD_ACCESS_FAULT:
        RT_EXCEPTION_TRACE(
            "exception: load access fault happened, epc=%08x, mdcause=0x%x\n",
            epc,
            mdcause
        );
        switch (mdcause & 0x07) {
    ...
    case MCAUSE_STORE_AMO_ADDR_MISALIGNED:
        RT_EXCEPTION_TRACE(
            "exception: store amo address was misaligned, epc=%08x\n", epc
        );
        break;
    case MCAUSE_STORE_AMO_ACCESS_FAULT:
        RT_EXCEPTION_TRACE(
            "exception: store amo access fault happened, epc=%08x\n", epc
        );
        switch (mdcause & 0x07) {
        case 1:
            RT_EXCEPTION_TRACE("mdcause: ECC/Parity error\r\n");
    ...
    rt_kprintf(
        "cause=0x%08x, epc=0x%08x, ra=0x%08x\n", cause, epc, s_stack_frame->ra
    );
    while (1) {
    }
}
    ...
    if (mcause & CSR_MCAUSE_INTERRUPT_MASK) {
        ...
        case IRQ_M_EXT:
            ...
            if (irq_index > 0) {
                ...
                /* Complete interrupt */
                __plic_complete_irq(
                    HPM_PLIC_BASE, HPM_PLIC_TARGET_M_MODE, irq_index
                );
            }
            break;
