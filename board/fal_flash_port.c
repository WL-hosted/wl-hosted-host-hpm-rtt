...
/* XPI NOR 配置选项（FCB）固定位于 XPI0 + 0x400（.nor_cfg_option 区），
   ROM 启动时按该地址读取；App 不再内置硬编码值，启动后按固定地址读取，
   与 bootloader/port/fal/fal_flash_port.c（xpi0_option[4]）保持一致 */
#define BOARD_APP_XPI_NOR_CFG_OPT_ADDR (FLASH_BASE + 0x400UL)
...
struct fal_flash_dev onchip_flash = {
     ...
     *  - addr 取链接脚本导出的 FLASH_BASE（XPI0 flash 基址，若改接 XPI1
     *    则改链接脚本中 __flash_base__ 即可）
...
FAL_RAMFUNC static int init(void) {
    rt_kprintf("Initializing FAL for XPI0 NOR Flash...\n");
    int ret = RT_EOK;
    xpi_nor_config_option_t cfg_option;
    const volatile rt_uint32_t *xpi0_opt =
        (const volatile rt_uint32_t *)BOARD_APP_XPI_NOR_CFG_OPT_ADDR;
    cfg_option.header.U = xpi0_opt[0];
    cfg_option.option0.U = xpi0_opt[1];
    cfg_option.option1.U = xpi0_opt[2];

    FAL_ENTER_CRITICAL();
    hpm_stat_t status = rom_xpi_nor_auto_config(
        BOARD_APP_XPI_NOR_XPI_BASE, &s_flashcfg, &cfg_option
    );
    FAL_EXIT_CRITICAL();
...
FAL_RAMFUNC static int read(long offset, rt_uint8_t *buf, rt_size_t size) {
    if (!flash_ready || offset < 0 || (rt_uint32_t)offset + size > onchip_flash.len) {
        return -RT_ERROR;
    }
    rt_uint32_t flash_addr = onchip_flash.addr + offset;
...
FAL_RAMFUNC static int write_unaligned_page_data(
    long offset, const rt_uint32_t *buf, rt_size_t size
) {
    hpm_stat_t status;
...
FAL_RAMFUNC static int write(
    long offset, const rt_uint8_t *buf, rt_size_t size
) {
    if (!flash_ready || offset < 0 || (rt_uint32_t)offset + size > onchip_flash.len ||
        size == 0U) {
        return -RT_ERROR;
    ...
    rt_uint32_t offset_in_page = (rt_uint32_t)offset % page_size;
    if (offset_in_page != 0) {
        rt_uint32_t write_size_in_page = page_size - offset_in_page;
        rt_uint32_t write_page_size = MIN(write_size_in_page, (rt_uint32_t)size);
    ...
    while (remaining_size > 0) {
        write_size = MIN((rt_uint32_t)remaining_size, sizeof(buf_32));
...
FAL_RAMFUNC static int erase(long offset, rt_size_t size) {
    ...
    while (aligned_size > 0) {
        FAL_ENTER_CRITICAL();
        if (((rt_uint32_t)offset % block_size == 0) &&
